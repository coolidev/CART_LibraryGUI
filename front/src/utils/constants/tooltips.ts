export const TooltipList = {
    ...
    eccentricity: "Describes the shape of an ellipse. A value of 0 represents a circular orbit; values greater than 0 but less than 1 represent ellipses of varying degrees of ellipticity", 

    argumentOfPerigee: "The angle from the ascending node to the eccentricity vector measured in the direction of the satellites motion. The eccentricity vector points from the center of the Earth to perigee with a magnitude equal to the eccentricity of the orbit.", 
    ...
    trueAnomaly: "The angle between lines drawn from the center of mass to a planet, and the perihelion point, where the planet comes closest to the sun", 

    serviceType: "The means of any radiocommunication service involving the use of one or more satellites", 

    serviceThroughputRequirements: "The amount of data generated by and received by the mission user platform in a given unit of time whether status telemetry, command uploads, or mission data that needs to be transported",

    dataVolume: "The volume of data in a single file or file system can be described by a unit called a byte", 

    data_Rate: "The amount of data that can be received, processed, and transmitted by a satellite", 

    standardsCompliance: "Physical layer standards that support high data-rate satellite communications in the presence of RF impairments in space by providing specifications for framing structure, channel coding, modulation systems, and spectrum efficiency",

    meanNumberOfContactsPerOrbit: "Is representative of how many network contacts a user would experience per orbit based on user orbit relative to the commercial systems configuration", 

    meanRFContactDuration: "The average duration of an individual RF coverage contact event, measured in seconds", 

    averageGap: "The average gap in RF coverage the user will experience, measured in minutes",

    maxGap: "The maximum gap in RF coverage the user will experience, measured in minutes", 

    meanResponseTime: "Measures, on average, how long a user might have to wait for the next RF coverage connection, based on the distribution of gap durations in the observed scenario", 

    serviceEfficiency: "How persistent access to a network is. Typically expressed as a ratio or percent; however, qualitive terms are often used", 

    gain: "Describes how much power is transmitted in the direction of peak radiation to that of an isotropic source", 

    EIRP: "The Effective Isotropic Radiated Power (EIRP) in dBW",

    polarizationType: "The direction of the electromagnetic fields produced by the antenna as energy radiates away from it. These fields determine the direction in which the energy moves away from or is received by the antenna.", 

    polarizationLoss: "The reduction in signal strength due to depolarization of a transmitted electromagnetic signal due to channel effects along the path between the transmit and receive antennas, in (dB)", 

    pointingLoss: "The reduction in signal strength due to the antenna pointing error or antenna misalignment in (dB)", 

    meanRFCoverage: "The duration that the user satellite has enough signal power to close the communication link with the desired system, over a defined period",

    transmitterPower: "The actual amount of power in (Watts) of the radio frequency energy that a transmitter produces at its output", 

    otherLosses:"Any other associated system losses that have not been considered",

    modulation: "The process of varying one or more properties of a periodic waveform, called the carrier signal, with a separate signal called the modulation signal that typically contains information to be transmitted", 

    coding: "A technique that adds redundant information to a message to support error detection (and error correction in some cases) on the transmitted signal and make it more robust against noise",

    frequencyBand: "A specific range of frequencies in the radio frequency (RF) spectrum, which is divided among ranges from very low to very high frequencies",

    centerFrequency: "Measurement of a frequency based on the halfway point of an electromagnetic signal with a certain bandwidth from its minimum value to its maximum value", 

    semiMajorAxis: "Radius of Earth + User Satellite altitude"
};
