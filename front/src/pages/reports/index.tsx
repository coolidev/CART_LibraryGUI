...
import { convertPlotlyToURI, image } from './utils/convertImage';
...
import { report as technicalReport } from './templates/cosmosTechnicalReport';
...
const dialogMessages = {
  visualizer: 'This report requires a snapshot of the Visualizer panel which is currently out of view. Please bring it back into view to generate this report.',
  coverage: 'This report requires snapshots of RF Coverage plots which are currently not loaded in the Analytics tab. Please switch your view to RF coverage and try again.',
  mac: 'Your report has been successfully generated. Please note that some images may not appear when viewing your report in Microsoft Word for Mac due to incompatibilities with embedded images. This will be resolved in a future release of CoSMOS. To view the images in the document, please load the report in a cloud-based document viewer such as SharePoint.'
}
...
const Reports: FC<ReportProps> = ({
  ...
  const { performancePanel, analyticsPanel, linkBudget, comparePanel, modCodOptions, frequencyBandOptions, antennaOptions, analyticsView } = useSelector(state => state.results);

  const [results, setResults] = useState<ResultsState>({performancePanel: performancePanel, analyticsPanel: analyticsPanel, linkBudget: linkBudget, comparePanel: comparePanel, modCodOptions: modCodOptions, frequencyBandOptions: frequencyBandOptions, antennaOptions: antennaOptions, analyticsView: analyticsView});

  const [reportSelection, setReportSelection] = useState<Report>(reports[0]?reports[0]:{name: '', html: '', requiresResults: true, needsVisualizer: true, description: null, fileType: 'DOCX', analyticsView: ''});
  ...
  const [libraryRef, setLibraryRef] =
    useState<{ [key: number]: LibraryValue }>(null);
  const [visHiddenDiag, setVisHiddenDiag] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [progress, setProgress] = useState<number>(0);
  const [statusMessage, setStatusMessage] = useState<string>("Generating Report...");
 // const [successDiag, setSuccessDiag] = useState<boolean>(false);
  ...
  useEffect(() => {
    setResults({performancePanel: performancePanel, analyticsPanel: analyticsPanel, linkBudget: linkBudget, comparePanel: comparePanel, modCodOptions: modCodOptions, frequencyBandOptions: frequencyBandOptions, antennaOptions: antennaOptions, analyticsView: analyticsView });
  ...
  useEffect(() => {
    if(visualizerListener){
      setTimeout(function() {
        if (visualizerListener) {
          let visualizerCache = {varName: 'VISUALIZER', html: `<p>[Visualizer Image Not Available]</p>`};
  ...
  async function generateImages() {
    ...
    Object.keys(PLOT_LIST).map((imageKey) => {
      imageCache = null;
      let size = (imageKey.includes('RFCoverage') || imageKey.includes('CoverageStatistics') || imageKey.includes('CoverageDistribution') || imageKey.includes('CoverageRunningAverage'))
        ? 450
        : 450;

      convertPlotlyToURI(imageKey + 'plotly',size).then(result => {
        imageCache = {varName: PLOT_LIST[imageKey].replaceAll('$',''), html: result};
  ...
  useEffect(() => {
    ...
    setStatusMessage("Generating Report...");
    if(codingRef){newPct += 8;}
    if(modulationRef){newPct += 8;}
    if(freqBandRef){newPct += 8;}
    if(gsRef){newPct += 8;}
    if(antennaRef){newPct += 8;}
    if(libraryRef){newPct += 10;}    
    setProgress(newPct);
    ...
    if (
      ...
      resetLoadingBar();
      //setSuccessDiag(true);
      if (window.navigator.appVersion.indexOf('Mac') !== -1 && currentReport === technicalReport){
  ...
  const updateSelection = (event): void => {
    ...
    setReportSelection(myReport?myReport:{name: '', html: '', requiresResults: true, needsVisualizer: true, description: null, fileType: 'DOCX', analyticsView: ''})
  }
  ...
  const generate = (): void => {
    ...
    if(analyticsView!=='coverage' && reportSelection.analyticsView==='coverage'){
      setVisHiddenDiag(true);
