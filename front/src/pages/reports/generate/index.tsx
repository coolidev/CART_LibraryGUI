...
export function generateReport(report: Report, requiresResults: boolean, reportType: string, images: reportImages[], project: Project, state: State, results: ResultsState, reportData: reportInfo){
  if((!results.analyticsPanel || !results.linkBudget || !results.performancePanel) && requiresResults){
    let missingResults = '';
    ...
    alert(`CoSMOS Reporting has not yet received results for:${missingResults}. Please ensure that your analysis has completed before generating this report.`);
    return;
  }

  const html = insertReportVariables(report, images, project, state, results, reportData);
