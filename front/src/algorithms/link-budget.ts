...
const computeLinkBudget = (systemParams: SystemParams, linkParams: LinkParams, state: State, id: number, previousLinkBudget: LinkBudgetRow[]) => {
    ...
    if (miscellaneousLossRow) {
        ...
        eirp_dBW += miscellaneousLosses_dB;
    }
    ...
    if (systemParams.networkType === 'dte') {
        ...
        linkBudget['freeSpaceLoss_dB'] = 32.45 + 20 * Math.log10(systemParams.rtnLinkFreqMHz) + 
        ...
        linkBudget['polarizationLoss_dB'] = systemParams.polarizationLoss_dB;
        linkBudget['miscellaneousLosses_dB'] = miscellaneousLosses_dB ?? 0;
    
        const prec_dBW = computePrec(
        ...
        linkBudget['netEbNo_dB'] = ebNoIntoDemod_dB - systemParams.implementationLoss;
